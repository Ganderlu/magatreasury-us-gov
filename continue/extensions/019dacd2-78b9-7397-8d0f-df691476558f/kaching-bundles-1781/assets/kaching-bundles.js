...
}, j = Ut() ? window.localStorage : window.sessionStorage, Z = () => new URLSearchParams(window.location.search).get("kaching");
let tt;
const Ht = () => (tt === void 0 && (tt = Z() === "off"), tt);
let et;
const W = () => (et === void 0 && (et = Z() === "debug"), et);
let nt;
const Bt = () => (nt === void 0 && (nt = Z() === "dev"), nt);
let it;
const Qt = () => (it === void 0 && (it = Z() === "info"), it), zt = async (i, t, e, n, r) => {
  try {
    const a = "kaching_visited_deal_blocks", o = j.getItem(a), s = o ? JSON.parse(o) : [];
    if (s.includes(t))
      return;
    s.push(t), j.setItem(a, JSON.stringify(s)), await fetch("https://bundles-stats.kachingappz.app/impressions", {
      method: "POST",
      headers: {
...
}, mt = /* @__PURE__ */ new Set(), k = async (i, t = {}, e = 1) => {
  if (d("sendStorefrontEvent", { name: i, data: t }), mt.has(i) || (mt.add(i), Math.random() > e))
    return;
...
function w(i, t = null) {
  !Qt() && !W() && !Bt() || console.info("[Kaching Bundles]", i, t);
}
const Kt = () => {
  const i = (n) => {
...
}, O = (i, t, e) => i.addEventListener(t, e), X = (i) => document.createElement(i), Wt = (i, t) => i && i.classList.add(t), P = (i, t, e) => i.setAttribute(t, e), M = (i) => i.dataset, z = (i) => {
  const t = A(i);
  if (!t)
    return;
  ...
  switch (n) {
    ...
    case "excluded-products": {
      ...
      for (; c.length < s; ) {
        ...
        if (c.push(...g.products.nodes.map((p) => p.id)), !g.products.pageInfo.hasNextPage) break;
...
`, J = async (i, {
  ...
  `, y = o ? "app--2935586817--kaching_bundles" : "$app:kaching_bundles", g = [], p = Math.min(s, 250);
  for (let h = 0; h < c.length; h += p)
    g.push(c.slice(h, h + p));
  const _ = (h) => i.query(f, {
    variables: {
      productGIDs: h,
  ...
  if (l) {
    const h = await Promise.allSettled(
      m.map(async (S) => {
        ...
        return kt(b, i.shopifyDomain);
      })
    ), C = [];
    for (const S of h)
      S.status === "fulfilled" ? C.push(S.value) : l(
  ...
  const e = i.variants.nodes.map(
    ({ unitPriceMeasurement: p, ..._ }) => {
      ...
      return {
        ...
        options: _.selectedOptions.map((C) => C.value),
        imageId: _.image ? Number(_.image.id.split("/").pop()) : null,
        image: ((u = _.image) == null ? void 0 : u.url) || null,
        sellingPlans: ((m = _.sellingPlanAllocations) == null ? void 0 : m.nodes.map((C) => ({
          id: Number(C.sellingPlan.id.split("/").pop()),
          price: C.priceAdjustments.length > 0 ? Math.round(Number(C.priceAdjustments[0].price.amount) * 100) : Math.round(Number(_.price.amount) * 100)
        }))) || [],
    ...
    const u = p.optionValues.map((m) => {
      ...
      return {
        ...
        swatch: {
          ...
          image: ((b = (S = (C = m.swatch) == null ? void 0 : C.image) == null ? void 0 : S.previewImage) == null ? void 0 : b.url) || null
        }
      };
    });
    return {
      ...
      name: p.name,
      position: _ + 1,
      optionValues: se(u, _, e)
    };
  }), r = (p) => {
    const _ = p.priceAdjustments[0];
    if (!_)
      return null;
...
const dt = "kaching_session_id", Ae = async () => {
  ...
  e && (j.setItem(dt, e), t.delete("preview_kaching_session_id"), i.search = t.toString(), window.history.replaceState({}, "", i.toString()));
}, H = () => j.getItem(dt) || De(), De = () => {
  const i = qe();
  return j.setItem(dt, i), i;
...
const Me = async (i) => {
  ...
  const e = (u) => {
    ...
    return {
      ...
      variantId: h.variantGIDs && ((C = h.variantGIDs) != null && C[0]) ? B(h.variantGIDs[0]) : null,
  ...
  return {
    ...
    marketId: (p = i.marketId) != null ? p : null,
  ...
  type: i,
  value: Number(t)
}, B = (i) => Number(i.split("/").pop()), xe = (i) => (F = i, {
  fetchDeals: $e
...
const ze = (i) => /\/cart\/change(\.js)?(\?|$)/.test(i), Je = (i) => {
...
class rn {
  ...
  registerDeal(t, e, n) {
    ...
    r.push({ getItems: n, product: e }), this._registeredDeals.set(t, r), d("CartInterceptor deal registered", {
      dealId: t,
      totalDeals: r.length
    });
  }
      ...
      if (this._isInterceptableUrl(r)) {
        ...
        if (s) {
          ...
          try {
            ...
            return d("Modified request failed, retrying with original", {
              error: l
  ...
  async _maybeRewriteCartChangeResponse(t, e) {
    ...
    if (r.quantity == null || !this._rewriteCartChangeEnabled && this._cartChangeDetectionReported)
      return t;
    try {
      const s = await t.clone().text(), l = We(s, r);
      if (!l.rewrite || (this._cartChangeDetectionReported || (this._cartChangeDetectionReported = !0, k("cart_change_rewrite_detected", {
  ...
  _interceptXHRRequests() {
    var s, l, c, f;
    const t = XMLHttpRequest.prototype.open, e = XMLHttpRequest.prototype.send, n = function(y, g, p = !0, _, u) {
      return St.set(this, g instanceof URL ? g.href : g), t.call(this, y, g, p, _, u);
    }, r = function(y) {
      var p, _;
      const g = St.get(this);
      if (g) {
        const u = x._getModifiedBody(g, y);
        if (u) {
  ...
  _getModifiedBody(t, e) {
    try {
      ...
      if (d("Original items", n), !n)
        return null;
      const r = this._findBundleItems(n);
      if (d("Bundle items", r), !r)
        return null;
      const a = this._mergeItems(n, r);
      if (d("Merged items", a), JSON.stringify(n) === JSON.stringify(a))
        return null;
  ...
  _findBundleItems(t) {
    ...
    if (!(a != null && a.length))
      return d("No registered deals found", { dealId: n }), null;
    for (const { getItems: l, product: c } of a) {
      const f = l();
      if (e ? f.some(
        (g) => t.some(
          (p) => p.id === g.id && p.properties.__kaching_bundles === g.properties.__kaching_bundles
        )
...
const an = (i) => typeof i == "string" ? i : i instanceof URL ? i.href : i && typeof i == "object" && "url" in i ? i.url : null, x = new rn(), K = async (i, t, e, n) => {
  var l;
...
}, sn = (i) => i.map((e) => e.swatchOptions || []).reduce((e, n) => e.concat(n), []).filter((e) => e != null).filter((e) => e.swatchType !== "default"), on = (i, t) => {
...
class ln {
  constructor(t, e, n, r, a, o, s) {
    ...
    if (this._items = [], this._clickedAddToCartBeforeFormSubmit = !1, this._isFirstVariantChange = !0, w("Deal block id:", a.id), w("Deal block settings", a), w("Config", n), w("Product", o), w("Deal block widget", { widget: t }), w("Cart form", { form: (l = e.addToCartForm()) == null ? void 0 : l.form }), w("Add to cart button", {
      ...
      const p = (y = this._productBlock.addToCartForm()) == null ? void 0 : y.currentVariantId();
      p && this._product.variants.some((_) => _.id == p) && (g = p), this._currentVariantId = g;
    } else
  ...
  _listenForFormSellingPlanChange() {
    ...
    if (t) {
      ...
      t.onSellingPlanChange((e) => {
        d("listenForFormSellingPlanChange", e), P(
          this._dealBlockElement,
          "selling-plan-id",
          e ? String(e) : ""
        );
  ...
  _changeQuantityInput() {
    ...
    d("_updateQuantityInput", r), t.value = String(r), this._globalConfig.shopifyDomain === "119a01-bf.myshopify.com" && t.dispatchEvent(new Event("input", { bubbles: !0 })), t.dispatchEvent(new Event("change", { bubbles: !0 })), setTimeout(() => {
  ...
  _listenForAmountDiscountExceedsPrice() {
    document.addEventListener(
      ...
      (t) => {
        ...
        k("amount_discount_exceeds_price_v3", {
          dealBlockId: this._dealBlockSettings.id,
          productId: this._product.id,
          country: this._country,
  ...
  _listenForUpsellSubscriptionInherited() {
    document.addEventListener(
      ...
      (t) => {
        ...
        k("upsell_subscription_inherited", {
          dealBlockId: this._dealBlockSettings.id,
          productId: this._product.id,
          upsellType: e
        });
  ...
  _listenForAddToCartClick() {
    ...
    const e = () => window.kachingBundlesDisableAddToCartHandling ? !1 : this._dealBlockSettings.skipCart || this._isUpcartAppEnabled() || this._isOpusAppEnabled() || this._isKrakenCartAppEnabled() ? !0 : this._globalConfig.featureFlags.intercept_cart_request ? !1 : this._items.length > 1, n = async () => {
      ...
      if (this._dealBlockSettings.skipCart) {
        await this._addAllItemsToCart(), window.kachingCartApi && (d("Kaching Cart update tiered promotions bar"), await window.kachingCartApi.updateTieredPromotionsBar()), window.location.href = Y("checkout");
        return;
      }
...
function cn(i, t, e) {
  i.currencyRate !== 1 && t.subscriptionsEnabled && e.sellingPlans.length > 0 && t.dealBars.some((n) => un(n)) && k(
...
class dn {
  ...
  _registerClickHandler() {
    ...
    const t = async (n) => {
      ...
      if (this._ignoreClick || w("Add to cart button clicked"), d("AddToCartButton#interceptClick", {
        conditionMet: r,
        preventDefault: a,
...
class R {
  ...
  _clickOptionWrapper(t, e) {
    ...
    return r ? (d("VariantPicker#_clickOptionWrapper", r), r.click(), !0) : !1;
  }
  ...
  _clickButton(t, e) {
    ...
    return r ? (d("VariantPicker#_clickButton", r), r.click(), !0) : !1;
  }
}
class fn {
  constructor(t, e, n) {
  ...
  _observeAddToCartButtonRemoval() {
    this._addToCartButton && L(this._addToCartButton.button, () => {
      ...
      t ? (this._addToCartButton.replaceButton(t), this._moveAddToCartButtonOutOfQuantityElement()) : this._addToCartButton = null, k(
...
class Tt {
  ...
  async _initializePlaceholder(t, e, n, r) {
    var p, _;
    const a = e.id;
    this._globalConfig.webPixel ? (d("Tracking view with web pixel", window.Shopify.analytics), Dt(() => {
    ...
    }, s = (_ = this._translations) == null ? void 0 : _.dealBlocks[n.id], l = { ...o, ...s }, c = I(
    ...
    const y = new fn(t, f, n);
    y.initialize();
    const g = X(
      "kaching-bundles-block"
    );
    for (g.setAttribute("data-instant-styles", "none"), new ln(
      g,
      y,
      f,
      l,
    ...
    t.appendChild(g), M(t).initialized = "true", _n(g), mn(
      this._globalConfig,
      g,
      n,
      l,
  ...
  async _fetchPlaceholdersData(t) {
    ...
    const e = t.map(
      ...
      n.map((b) => {
        const E = JSON.parse(b.textContent);
        return E.variants.length === 250 ? null : [E.id, E];
      }).filter((b) => b !== null)
    ), a = e.filter(
      (b) => !r.has(b)
    ), o = I(
    ...
    ), u = new Map([...r, ..._]), m = H(), h = /* @__PURE__ */ new Map();
    for (const b of t) {
      const E = Number(b.getAttribute("product-id")), D = u.get(E);
      if (!D)
        continue;
      const ht = ((C = s.get(E)) == null ? void 0 : C.filter((v) => v)) || p;
      ht.sort((v, q) => {
        const ft = !!v.marketId, jt = !!q.marketId;
        return ft === jt ? 0 : ft ? -1 : 1;
      });
      let T = ce(ht, D);
      d("applicableDealBlocks", T), T = T.filter((v) => v.marketId ? v.marketId === this._globalConfig.marketId : !0), T = T.filter((v) => {
        ...
        return v.abTestVariantNumber === q;
      ...
      const Q = (S = b.getAttribute("manual-deal-block-id")) == null ? void 0 : S.trim();
      if (Q) {
        const v = T.findIndex(
          (q) => q.id === Q || q.nanoId === Q
        );
...
const Et = (i) => {
  ...
  if (!i)
    return;
  w("Shopify domain:", i.shopifyDomain), Jt();
...
document.readyState === "loading" ? (d("Waiting for DOMContentLoaded"), document.addEventListener("DOMContentLoaded", At)) : At();
